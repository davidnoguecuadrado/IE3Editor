...
namespace Inazuma_Eleven_Toolbox.Forms
{
    public partial class Save_Editor : Form
    {
        ...
        void LoadPlayer()
        {
            ...
            IECommonDictionary Dic = new IECommonDictionary();

            foreach (var x in Dic.MoveTohexa) {
                comboBox5.Items.Add(x.Value);
            ...
            {                
                ...
                ushort MinStamina = block[0x28];
                ushort MaxStamina = block[0x29];
                ushort Maxtotal;
                    
                Maxtotal = BitConverter.ToUInt16(block.Skip(0x3C).Take(2).ToArray(), 0);
                
                ushort StatsTotal = (ushort)(MaxKick + MaxBody + MaxGuard + MaxControl + MaxSpeed + MaxGuts + MaxStamina);
                
                short Freedom = (short)(Maxtotal - StatsTotal);
        ...
        void SetAttrivutes(int PlayerIndex)
        {
            int PlayerOffset = (PlayerIndex * length);
            byte[] block = ModifiedBlock.Skip(PlayerOffset + PlayerStartOffset).Take(length).ToArray();
            ...
            TrainedValue[0] = Convert.ToSByte(numericUpDown8.Value);
            TrainedValue[1] = Convert.ToSByte(numericUpDown9.Value);
            TrainedValue[3] = Convert.ToSByte(numericUpDown10.Value);
            TrainedValue[2] = Convert.ToSByte(numericUpDown11.Value);
            TrainedValue[4] = Convert.ToSByte(numericUpDown12.Value);
            TrainedValue[6] = Convert.ToSByte(numericUpDown13.Value);
            TrainedValue[5]= Convert.ToSByte(numericUpDown14.Value);

            for (int i = 0; i < 7; i++)
            {
                block[0x4F + i] = (byte)TrainedValue[i];
            }
        ...
        {
            int PlayerOffset = (DataGridviewIndexFromCell(dataGridView1.CurrentRow.Index) * length);
            byte[] block = ModifiedBlock.Skip(PlayerOffset + PlayerStartOffset).Take(length).ToArray();
            ...
            Int32 save = Dic.MoveTohexa.FirstOrDefault(x => x.Value == comboBox5.Text).Key;
        ...
        private void Save_Editor_Load(object sender, EventArgs e)
        {
        ...
        private void comboBox6_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Int32 save = Dic.MoveTohexa.FirstOrDefault(x => x.Value == comboBox6.Text).Key;
